...
@CommandPkg(label = "Converts String to Date ", name = "stringtodate",
        ...
        node_label = "Converts String to Date", icon = "pgk.svg",comment = true  , background_color =  "#86c2ff" ,
        return_type=DataType.DATETIME, return_label="Date", return_required=true)
public class StringtoDate {
    ...
    public DateTimeValue  action(@Idx(index = "1", type = TEXT)  @Pkg(label = "Date String" , default_value_type = STRING) @NotEmpty String datestring,
                                 @Idx(index = "2", type = TEXT) @Pkg(label = "Source Format"  , default_value_type = STRING ) @NotEmpty String format,
                                 @Idx(index = "3", type = TEXT) @Pkg(label = "Locale"  , default_value_type = STRING , default_value = "en") @NotEmpty String formatlocale) throws Exception
     {
		...
		SimpleDateFormat dt = new SimpleDateFormat(format,locale); 
